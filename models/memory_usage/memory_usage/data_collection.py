...
from dowser.profiler import Profile
...
def _generate_many_evenly_spaced(
    ...
    logger.info(
        f"Generating {amount_of_datasets} sets of synthetic data with distributed shapes within max bounds "
        f"({max_inlines}, {max_crosslines}, {max_samples})"
    )

    filepaths = []
    output_dir = f"{dowser.context.config.output_dir}/{data_folder}"
    ...
    logger.info(
        f"Calculated {amount_of_generated_shapes} shapes to evenly generate data for"
    )
    ...
    for i in range(amount_of_generated_shapes):
        ...
        logger.info(
            f"Generating synthetic data {i+1}/{amount_of_generated_shapes} with shape "
            f"({num_inlines}, {num_crosslines}, {num_samples})"
        )

        filepath = generate_and_save_synthetic_data(
            num_inlines,
            num_crosslines,
            num_samples,
        ...
        logger.info(
            f"Finished generating synthetic data {i+1}/{amount_of_generated_shapes}. Data stored on: {filepath}"
        )
...
def _profile_single(handler: Callable, dataset_path: str) -> str:
    ...
    dowser.profile(handler, dataset_path)
...
def _parse_profiles(profiles: List[Profile]) -> Dict:
    ...
    logger.info("Parsing the peak data from profiles")
...
def _store_collected_data(data: Dict) -> None:
    ...
    output_dir = dowser.context.config.output_dir.parent
    output_path = f"{output_dir}/collected_data.json"

    logger.debug(f"Saving collected data to {output_path}")
