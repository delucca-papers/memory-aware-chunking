...
def watch_execution_time(
    ...
    module_logger.info("Started execution time watcher")
...
def __build_event_time_measurer(
    ...
    def event_time_measurer(next_event_name: EventName) -> float:
        ...
        if event_dispatcher.worker_exited():
        ...
        while True:
            if event_dispatcher.wait(next_event_name, timeout=0.5):
                break
            if event_dispatcher.worker_exited():
                module_logger.warn("Worker exited while waiting for event")
                exited = True
                break
