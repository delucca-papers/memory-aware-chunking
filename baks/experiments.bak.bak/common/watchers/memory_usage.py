...
def watch_memory_usage(
    ...
    if not is_watcher_started:
        module_logger.info("Started memory usage watcher")
    ...
    while True:
        if event_dispatcher.wait(next_event, timeout=0.5):
            module_logger.debug(f"Received event: {next_event.value}")
            break
        if event_dispatcher.worker_exited():
            module_logger.warning("Worker exited while waiting for event")
            worker_exited = True
            break
