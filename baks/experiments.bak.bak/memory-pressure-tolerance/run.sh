...
function save_input {
    ...
    echo "Number of Inlines, Number of Crosslines, Number of Samples, Starting Pressure, Step Size, Number of Iterations, Attributes, Output Directory, Cgroup Name, Log Level" >"${OUTPUT_DIR}/input.csv"
    echo "\"${NUM_INLINES}\",\"${NUM_CROSSLINES}\",\"${NUM_SAMPLES}\",\"${STARTING_PRESSURE}\",\"${STEP_SIZE}\",\"${NUM_ITERATIONS}\",\"${ATTRIBUTES}\",\"${OUTPUT_DIR}\",\"${CGROUP_NAME}\",\"${LOG_LEVEL}\"" >>"${OUTPUT_DIR}/input.csv"
    echo
}
