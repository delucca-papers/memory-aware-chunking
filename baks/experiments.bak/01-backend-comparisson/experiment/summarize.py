...
def __get_results(directory: str, backend: str, heading_lines: int = 6) -> pd.DataFrame:
    memory_usage_metrics_dir = os.path.join(directory, "metrics", "memory_usage")
    backend_memory_usage_result_files = os.listdir(memory_usage_metrics_dir)
    backend_memory_usage_file = list(
        filter(lambda x: backend in x, backend_memory_usage_result_files)
    )

    if len(backend_memory_usage_file) > 1:
        raise RuntimeError("More than one memory usage result file found")
    if len(backend_memory_usage_file) == 0:
        raise RuntimeError("No memory usage result file found")

    backend_memory_usage_result = open(
        os.path.join(memory_usage_metrics_dir, backend_memory_usage_file[0]),
        "r",
    )
